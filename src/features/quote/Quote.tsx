...
import { getQuoteFromState, limpar, getQuoteStatus, getQuoteFromAPI } from "./quoteSlice";
import { obterMensagem } from "./utils";

function Quote() {
  const [valorInput, setValorInput] = useState("");
  const { quote = "", personagem = "" } = useAppSelector(getQuoteFromState, shallowEqual) || {};
  ...
  return (
    <ContainerQuote>
      ...
      <Input aria-label="Author Quote" value={valorInput} onChange={(e) => setValorInput(e.target.value)} placeholder="Digite o nome do autor" />
      <Botao aria-label={valorInput ? "Obter Citação" : "Obter citação aleatória"} onClick={onClickGetQuote}>
